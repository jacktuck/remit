...
function Request (base, type, options) {
  ...
  const expirationGroup = (options.schedule) ? (+options.schedule) : options.delay
  ...
  Object.defineProperty(request, '_queueName', {
    get: function () {
      debug('this._options', this._options)
      if (!isNaN(this._options.delay) || this._options.schedule && !isNaN(new Date(this._options.schedule).getTime())) {
        return `d:${remit._options.exchange}:${this._options.event}:${expirationGroup}`
      } else {
  ...
  request.queueInfo = QueueInfo.bind(this, request._queueName)
