...
const QueueInfo = require('./QueueInfo')
...
function Request (base, type, options) {
  ...
  let request = function () {
    ...
    const expiration = (options.schedule) ? (+options.schedule - now) : options.delay
      ...
      if (expiration > 0) {
        demitQueue = new Promise((resolve, reject) => {
          ...
          remit._workChannelPool.acquire().then((channel) => {
            ...
            return workChannel.assertQueue(request._queueName, queueOpts)
          }).then(() => {
  ...
  Object.defineProperty(request, '_queueName', {
    get: function () {
      let queueName
      let expirationGroup = (this._options.schedule) ? (+this._options.schedule) : this._options.delay

      debug('IN GETTER', '_options', this._options)
      debug('IN GETTER', 'expirationGroup', expirationGroup)

      if (!isNaN(this._options.delay) || this._options.schedule && !isNaN(new Date(this._options.schedule).getTime())) {
        queueName = `d:${remit._options.exchange}:${options.event}:${expirationGroup}`
      } else {
        queueName = options.event
      }

      debug('IN GETTER', 'SHOULD GIVE QUEUENAME OF', queueName)
  ...
  request.queueInfo = () => {
      return QueueInfo.call(this, request._queueName)
  }
